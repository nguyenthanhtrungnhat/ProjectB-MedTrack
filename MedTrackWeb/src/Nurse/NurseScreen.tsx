...
export default function NurseScreen() {
    ...
    const url = `localhost:3000/nurses/by-user/${userID}`;
    const roomsUrl = 'localhost:3000/rooms';
    ...
    useEffect(() => {
        ...
        const fetchCount = async () => {
            try {
                const res = await axios.get(`localhost:3000/api/schedules/${nurseID}`);
                const data = res.data;
    ...
    useEffect(() => {
        if (!nurseID) return;
        setLoading(true)
        axios.get(`localhost:3000/nurses/${nurseID}`)
            .then(response => {
